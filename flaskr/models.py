...
# 施設外就労ユニット
class Unit(db.Model, ModelMixInID):
    __tablename__ = 'units'
    __table_args__ = (
        db.PrimaryKeyConstraint('id'),
        {'mysql_engine': 'InnoDB'}
    ...
    id = db.Column(db.String(36), default=_get_uuid)
    caption = db.Column(db.String(64), nullable=False)  # ユニット名
    name = db.Column(db.String(64), nullable=False)     # 施設外就労企業名
    address = db.Column(db.String(128), nullable=False) # 所在地
    agree_in = db.Column(db.Date, nullable=False)       # 契約開始
    agree_out = db.Column(db.Date, nullable=False)      # 契約終了
    goal = db.Column(db.String(128), nullable=False)    # 目標・目的
    content = db.Column(db.String(128), nullable=False) # 作業内容
    def __repr__(self):
        return '<Unit: id={0.id}, caption={0.caption}>'.format(self)

# 施設外就労報告書
class UnitLog(db.Model, ModelMixInID):
    __tablename__ = 'unitlogs'
    __table_args__ = (
        db.PrimaryKeyConstraint('unit_id', 'yymm'),
        db.ForeignKeyConstraint(['unit_id'], ['units.id']),
        {'mysql_engine': 'InnoDB'}
    )
    unit_id = db.Column(db.String(36))
    yymm = db.Column(db.String(8))                   # 年月
    remarks = db.Column(db.String(128))              # その他
    def __repr__(self):
        return '<UnitLog: unit_id={0.unit_id}, yymm={0.yymm}>'.format(self)

# 施設外就労ユニット−利用者
class UnitPerson(db.Model):
    __tablename__ = 'unitpersons'
    __table_args__ = (
        db.PrimaryKeyConstraint('unit_id', 'person_id'),
        db.ForeignKeyConstraint(['unit_id'], ['units.id']),
        db.ForeignKeyConstraint(['person_id'], ['persons.id']),
        {'mysql_engine': 'InnoDB'}
    )
    ...
    def __repr__(self):
        return '<UnitPerson: unit_id={0.id}, person_id={0.person_id}>'.format(self)
